...
# 이상감지 렌더 함수 전체는 app/components/anomaly_monitor.py 로 이동 (AI 예측 페이지에서 사용).
# 임원 요약 보고서가 이상감지 요약을 유지하므로 fetch 함수만 계속 공유한다.
from app.components.anomaly_monitor import _fetch_anomaly_data
...
def _holiday_dates() -> frozenset:
    """DB_holiday.xlsx 기반 공휴일 집합(일일 메일 빌더와 동일 기준, 프로세스 내 캐시)."""
...
def _previous_business_day(d: date) -> date:
    """d 하루 전부터 주말·공휴일을 건너뛴 첫 평일 근무일. 일일 메일 기준일 규칙과 동일.

    예: 화요일이면 월요일, 월요일이면 지난주 금요일을 반환한다.
    """
    hols = _holiday_dates()
    prev = d - timedelta(days=1)
    while prev.weekday() >= 5 or prev in hols:
...
def _ratio_html(val, invert: bool = False, neutral: bool = False) -> str:
    """비율(비)을 퍼센트 문자열 + 색상 HTML로 반환.
    invert=True  : 높을수록 좋음 (생산량) → ≥100% 파란, <100% 빨강
    invert=False : 낮을수록 좋음 (원단위) → ≤100% 파란, >100% 빨강
    neutral=True : 좋음/나쁨 판단 없음 (사용량) → 중립색.
                   사용량 증감은 생산량 변동의 영향을 함께 받으므로
                   색으로 개선/악화를 단정하면 오독을 유발한다 (효율 판단은 원단위).
    """
            ...
            with cols[col_idx]:
                ...
                with st.expander("📄 데이터 테이블 상세보기", expanded=False):
                    ...
                    st.dataframe(
                        ...
                        hide_index=True,
                        column_config={
                            "생산량 (DB 실적, kg)": st.column_config.NumberColumn(format="%,.0f"),
                            usage_label: st.column_config.NumberColumn(format="%,.0f"),
                            "생산량 지수": st.column_config.NumberColumn(format="%.1f"),
...
def render_yoy_chart(base_date: date):
        ...
        with active_col:
            ...
            for j, (cv, pv) in enumerate(zip(curr_vals, prev_vals)):
                if cv is not None and pv is not None and pv > 0:
                    ...
                    # 색 규칙: 생산량 = 높을수록 좋음(＋파랑) / 원단위 = 낮을수록 좋음(－파랑)
                    # 사용량 = 중립 — 생산량 변동 영향이 섞여 있어 색으로 좋음/나쁨을
                    # 단정하면 오독을 유발한다 (효율 판단은 원단위 지표).
                    if ui_data_type == "생산량":
    ...
    if ui_data_type == "사용량":
        st.caption(
            "※ 사용량 증감률은 중립색으로 표시합니다 — 생산량 변동의 영향이 섞여 있어 "
            "증감만으로 개선/악화를 판단할 수 없습니다. 효율 판단은 '원단위'를 선택하세요."
        )
...
def _render_comparison_table(
    ...
    neutral_labels: set = None,
):
    """공장 계층 비교 테이블 렌더링.
    invert_labels: 비 컬럼 색상을 반전할 unit_label 세트 (생산량 등)
    neutral_labels: 좋음/나쁨 색을 입히지 않을 unit_label 세트 (사용량 4종 —
                    생산량 변동 영향이 섞여 있어 색 판단이 오독을 유발)
    """

    if invert_labels is None:
        invert_labels = set()
    if neutral_labels is None:
        neutral_labels = set()
    ...
    for unit_label in unit_labels:
        ...
        for factory in factory_order:
            ...
            is_neutral = unit_label in neutral_labels
            ...
            html += f'<td style="{td_base} background:{row_bg}; text-align:center;">{_ratio_html(r.get("전년동월_MTD비"), invert=is_inv, neutral=is_neutral)}</td>'
            ...
            html += f'<td style="{td_base} background:{row_bg}; text-align:center;">{_ratio_html(r.get("전년동기비"), invert=is_inv, neutral=is_neutral)}</td>'
            html += '</tr>'
    ...
    with st.expander("📄 상세 비교 테이블 보기 (원단위 / 사용량)", expanded=False):
        ...
        with c1:
            _render_comparison_table(unit_df, "유틸리티 원단위 현황", u_labels,
                                    base_date_str)
        with c2:
            # 사용량 4종은 중립색 — 생산량 증감 영향이 섞여 있어 색으로
            # 개선/악화를 단정할 수 없다 (효율 판단은 좌측 원단위 표).
            _render_comparison_table(usage_df, "생산량 및 사용량 현황", s_labels,
                                    base_date_str,
                                    invert_labels={"생산량(DB 실적)\n[ton]"},
                                    neutral_labels={
                                        "전력 사용량\n[kWh]", "연료 사용량\n[Nm³]",
                                        "용수 사용량\n[ton]", "폐수 사용량\n[ton]",
                                    })
        st.caption(
            "※ 사용량 증감비는 좋음/나쁨 색을 표시하지 않습니다 — 생산량 변동의 영향이 "
            "섞여 있어 효율 판단은 원단위 기준이 정확합니다."
        )
...
def _render_energy_composition(usage_df, base_date_str):
    ...
    # 값은 당해누계(YTD) — 이전 라벨 "(YYYY-MM)"은 해당 월 단독 비중처럼 읽혀
    # 데이터와 라벨이 어긋났음. 연간 누계임을 명시한다.
    st.markdown(f"""<div class="section-header"><span class="section-icon">🍩</span>
        공장별 에너지 사용 비율 <span style="font-size:0.82rem;color:{_TEXT_SECONDARY};">({bd.year}년 1월~{bd.month}월 누계)</span></div>""",
        unsafe_allow_html=True)
    if usage_df.empty:
...
def render_main_dashboard():
    ...
    # 기본 기준일 = 근무일 기준 D-1(주말·공휴일 제외). 일일 메일 기준일과 동일 규칙.
    # 예: 오늘 화요일이면 월요일, 월요일이면 지난주 금요일.
    prev_business_day = _previous_business_day(date.today())
    # 근무일 D-1이 DB 범위 안에 있으면 그 날을, 아니면 DB 최대일을 기본값으로 사용
    if db_min_date <= prev_business_day <= db_max_date:
        default_base = prev_business_day
    else:
        default_base = db_max_date

    # 기준일 설정 섹션 (가깝게 배치하기 위해 flex 컨테이너와 합침)
    col_info, col_label, col_date, col_exec, col_mail = st.columns([2.0, 0.4, 1.1, 1.0, 1.0])
    with col_exec:
        # One-Page 임원 요약 보기 — 인쇄/PDF 친화 뷰로 전환 (누구나 열람 가능)
        if st.button(
            "🖨️ 임원 요약",
            use_container_width=True,
            key="open_exec_summary_btn",
            help="현재 기준일의 1페이지 임원 요약 보고서를 엽니다. 브라우저 인쇄(Ctrl+P)로 PDF 저장 가능.",
        ):
            st.session_state["_exec_summary_open"] = True
            st.rerun()
    with col_date:
    ...
    # ── (이동됨) AI 예측 이상 알림 + 관리도 섹션 ──
    # 모델 정확도가 장기 안정화될 때까지 홈에서 제외 (2026-07 결정).
    # 렌더 코드는 app/components/anomaly_monitor.py 로 이동했고,
    # AI 에너지 분석 > 에너지 사용 예측의 "🚨 이상감지 현황" 탭에서 노출된다.
    # 임원 요약 보고서의 이상감지 요약(_fetch_anomaly_data)은 유지.
