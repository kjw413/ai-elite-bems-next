...
`.env`의 `MAIL_RECIPIENTS`로 근무일 기준 D-1의 6개 사업장 실적과 전주 동일 요일 대비 생산량·원단위 즉시 점검 대상을 HTML 메일로 발송합니다. 메일 생성 직전에 `RawDB_에너지.xlsx`와 `DB_생산실적.xlsx` 변경분을 각각 `energy_daily`, `production_daily`에 자동 반영하며, 생산량과 원단위 분모는 `DB_생산실적`만 사용합니다.

```bat
:: 기본 (근무일 기준 D-1, .env 설정)
