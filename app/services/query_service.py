...
def _apply_gwangju_correction(df: pd.DataFrame) -> pd.DataFrame:
    """광주(F30) 한정 분모 보정.

    Why: `energy_daily.mix_prod_kg`(RawDB_에너지 동기화 값)는 광주에서 자사 완제품의
    믹스 톤만 잡고 있고 **외부 판매용 재공품**(탈지분유·살균유·생크림·유크림믹스 등)
    분량이 빠져 있다. 프론트엔드는 이 raw 값에 DB_재공품(믹스 환산)을 합산해
    "재공품 실적을 합산한 실제 원단위" 분모로 표시해야 한다.

    How to apply: 광주 행만 골라 `mix_prod_kg ← raw_mix + wip_kg(환산)` 으로 더한
    뒤 4 종 원단위 컬럼을 재계산한다. raw 와 wip 모두 0 인 날은 분모 0 유지(원단위 NaN).
    다른 공장은 손대지 않는다.

    옵션 B(production_daily 로 raw 대체)는 production_daily 광주(F30) 적재 누락 시
    raw 값을 잃어버리는 부작용이 있어 채택하지 않는다. raw 가 부정확하더라도
    "raw + wip" 는 raw 의 정보를 보존하면서 누락된 재공품을 더하는 방향으로 작동한다.
    """
    if df is None or df.empty or "factory" not in df.columns:
        return df
    mask = df["factory"] == "광주"
    if not mask.any():
        return df
    ...
    d_from = pd.to_datetime(df.loc[mask, "date"]).min()
    d_to = pd.to_datetime(df.loc[mask, "date"]).max()
    try:
        bd = get_breakdown_daily("광주", d_from, d_to)
    except Exception as exc:
        logger.warning(f"[gwangju-correction] breakdown 로드 실패 — 원본 유지: {exc}")
        return df
    if bd.empty:
        return df
    ...
    wip_map = dict(zip(bd["date"], bd["wip_kg"].astype(float)))

    g_dates = pd.to_datetime(df.loc[mask, "date"]).dt.normalize()
    wip_add = g_dates.map(wip_map).fillna(0.0).astype(float)
    orig_mix = pd.to_numeric(df.loc[mask, "mix_prod_kg"], errors="coerce").fillna(0.0)
    corrected = orig_mix.values + wip_add.values
    df.loc[mask, "mix_prod_kg"] = corrected

    idx = df.loc[mask].index
    denom_ton = pd.Series(corrected / 1000.0, index=idx)
    for unit_col, usage_col in UNIT_CALC_MAP.items():
        if unit_col not in df.columns or usage_col not in df.columns:
            continue
        usage = pd.to_numeric(df.loc[mask, usage_col], errors="coerce")
        new_unit = (usage / denom_ton).where(denom_ton > 0)
        df.loc[mask, unit_col] = new_unit
    return df
...
def get_daily_data(
    ...
    # 광주 한정: RawDB_에너지의 mix_prod_kg(외주 부담 포함)를 자사 활동 기준
    # accounted_kg(완제품 + DB_재공품 환산)로 교체하고 원단위 4종 재계산.
    # 이후 전사/남양주 집계와 모든 프론트엔드 페이지가 자동으로 보정값을 본다.
    df = _apply_gwangju_correction(df)
...
def get_yoy_data(
    ...
    # 광주 한정: RawDB_에너지의 mix_prod_kg에 DB_재공품(외부 판매 재공품 환산)을 가산해
    # 일별/기간별 화면(get_daily_data)·메일 alert와 동일한 분모로 맞춘다. 월 집계 이전에
    # 일 단위 raw 행에 적용해야 SUM(생산톤)이 정합한다 (보정 누락 시 전년대비 누계 원단위가
    # 광주·전사에서 메일 YTD와 어긋남).
    df = _apply_gwangju_correction(df)
...
def get_factories() -> list[str]:
    ...
    표시 순서: 실공장(남양주1, 남양주2) → 집계 공장(남양주) → 김해 → 광주 → 논산.
