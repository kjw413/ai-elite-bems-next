...
============================================
.bat 또는 수동 실행:
    python tools/mail/run_daily_mail.py                 # 기본: 근무일 기준 D-1 (.env 설정)
...
def _sync_latest_production_data(log) -> int:
    """메일 생성 전에 DB_생산실적.xlsx 변경분을 production_daily에 반영한다."""
    if not _env_flag("DAILY_REPORT_SYNC_BEFORE_BUILD", True):
        log.info("메일 전 생산실적 자동 동기화 비활성화(DAILY_REPORT_SYNC_BEFORE_BUILD=false)")
        return 0

    try:
        from app.services.production_dw_sync_service import auto_sync_production_once
        result = auto_sync_production_once()
    except Exception as exc:
        log.exception(f"메일 전 생산실적 자동 동기화 예외: {exc}")
        return 7

    status = result.get("status") if result else "error"
    if status in ("error", "missing_source"):
        log.error(f"메일 전 생산실적 자동 동기화 실패: {(result or {}).get('message', status)}")
        return 7
    if status == "synced":
        log.info(f"메일 전 생산실적 자동 동기화 완료: {result.get('message')}")
    else:
        log.info(f"메일 전 생산실적 자동 동기화: {result.get('message', status)}")
    return 0
