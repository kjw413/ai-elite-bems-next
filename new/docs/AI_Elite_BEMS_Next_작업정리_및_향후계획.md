...
> **최종 갱신: 2026-07-15 / 기준 커밋: `4695f5f` / 후속 작업 진행 중**
>
...
> **다음 작업 우선순위:**
>
> 1. 브라우저에서 핵심 조회 화면 5종 smoke test
...
| 현재 진행 | 실제 사내 DB·브라우저·viewer 권한 동등성 검증 준비 |
| 이번 세션 완료 | npm 잠금·독립 `new/.venv`; 프런트 P0; 운영 생산량·F-code/ton·예측 결측·감사/CORS·조회 전용 DB 경로 백엔드 P0/P1; TypeScript·Next.js 빌드·웹/API HTTP smoke·Python 문법·백엔드 단위 테스트 17건 통과 |
| 바로 다음 작업 | 실제 DB에서 legacy 수치 비교, 다른 PC viewer 403, 브라우저 차트·반응형 확인 |
| 이후 남은 작업 | AI 보고서·관리자 UI 연결, 실제 DB 수치 동등성·권한·업로드 검증, 운영 안정화 |
...
- `python -m unittest discover -s backend/tests -v`로 helper·윤년·Origin 확장·생산 집계/F10·예측 완전성·point 예측·viewer 403·필수 조회 계정 테스트 17건 통과
...
- Node.js `v24.14.0`, npm `11.9.0` 환경에서 `npm install --no-audit --no-fund` 성공
- npm 패키지 66개 설치 및 `package-lock.json` 생성 완료
- 잠금파일 기준 `npm ci --no-audit --no-fund` 재현 설치 성공(61개 패키지)
...
- `npm ci` 직후 빌드와 최종 변경 후 재빌드 모두 통과(최종 컴파일 3.0초)
- 생성 결과: 메인 경로 `/` 정적 렌더, First Load JS 약 230 kB
...
현재 `new/backend/tests/test_server_helpers.py`에 DB 비의존 단위 테스트 17건을 추가했다.
...
- 이번 작업 기준 커밋: `4695f5f`
- `origin/main`과 로컬 HEAD가 `4695f5f`로 일치함을 확인
- 이번 세션의 P0 개선은 아직 로컬 작업 트리에 있으며 커밋·푸시는 수행하지 않음
...
- [x] 기준 커밋 `4695f5f`까지 `origin/main` 반영
- [ ] 이번 세션의 `new/` 개선 변경 커밋·푸시
- [ ] MySQL 서비스 실행
...
npm 의존성 설치, TypeScript 검사와 프로덕션 빌드는 완료됐다. 이어서 브라우저 smoke
test를 통과시켜 현재 프런트엔드의 실제 렌더링과 반응형 동작을 확인해야 한다. 그다음 보고서·관리자
