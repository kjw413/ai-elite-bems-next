...
### Phase 4 — UI 완성·세련화 & legacy 퇴역 ◐ (개발분 2026-07-16 완료, 병행 비교·퇴역 남음)

- 잔여 기능 ✅: 생산실적 **월별/기간별/연간 모드**(`GET /api/v1/production`의
  `mode`/`date_from`/`date_to`, 완전 월 범위에서만 계획 지표 표시, 연간 Burn-up,
  기타(ETC) 카테고리), **CSV 내보내기**(`lib/bems-csv.ts`, utf-8-sig BOM으로
  legacy `csv_download` 동등 — 대시보드 7일 추이·전년비, 에너지 일별·공장별,
  원단위 추이·매트릭스, 생산 일별·품목 순위, 예측 이력). 재학습·기상 동기화 UI는
  Phase 3에서 완료.
- 디자인 시스템 ✅: 공통 차트 팔레트를 CSS 변수(`--chart-*`)로 통일하고
  dataviz 검증기로 라이트/다크 표면 모두 검증(다크는 amber만 #d97706로 조정해
  전 검사 통과, 라이트 amber 대비 WARN은 같은 화면 표·범례로 구제).
  **다크모드**: `<html data-theme>` + OS 설정 자동 감지 + 상단바 토글
  (localStorage 유지), 인쇄는 항상 라이트 값 사용(@media screen 스코프).
