...
- DB의 원본 `energy_daily.mix_prod_kg`는 운영 분모로 사용하지 않는다.
- 운영 생산량은 `production_actual_service`에서 **DB_생산실적 완제품 + 판매용 반제품(WIP)**으로 만든다. 판매용 WIP는 **광주공장만** 합산한다. `DB_재공품.xlsx`의 7품목(`260014`, `260016`, `260039`, `260042`, `260047`, `260351`, `260352`)과 내부 MIS 사정으로 `DB_생산실적.xlsx`에 기록되는 2품목(`129998` 탈지분유(수), `129999` 생크림(35%)(수))을 재공품으로 분류하며, `129998`에는 탈지분유 환산계수 `10.91954`, `129999`에는 `1.0`을 적용한다.
- 따라서 광주 원단위는 반드시 이 WIP 포함 운영 생산량을 분모로 해석한다. `production_daily` 완제품만으로 광주 효율 "악화"를 단정하지 않는다.
