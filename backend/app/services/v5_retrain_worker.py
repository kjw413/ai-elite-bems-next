...
#
# [2026-09-16 사용자 제안 — 미검증 공정 가설]
# 사용자 설명상 광주는 관리 대상 공장 중 유일하게 스낵류를 생산하는 공장이다.
# 건조 스낵 공정은 액상 믹스 공정과 달리 품목군 전환마다 같은 빈도로 CIP를
# 실시하지 않을 수 있다. 이 경우 스낵 품목 수가 늘어도 실제 세정 부하가
# 비례해 늘지 않아, 공장 전체 활성 WIP 품목 수가 연료/용수의 CIP 부하를
# 나타내는 대리변수로서 약해지고 예측 성능을 악화시켰을 가능성이 있다.
# 이는 원인으로 확정된 사실이 아니며, 위 주석의 분포 차이/검증 기록과 구분한다.
# 광주 내 모든 라인에 같은 세정 특성을 가정하거나, 활성 품목 수를 실제
# CIP 횟수와 일대일 대응시키지 않는다. 기존 광주 제외 정책은 그대로 유지한다.
# 후속 확인: 품목 코드의 스낵/액상 공정군 매핑과 라인별 전환·세정 이력을 확인하고,
# 생산량·날씨·제품 구성 영향을 함께 고려해 공정군별 활성 품목 수의 설명력을 비교한다.
# 변수 미사용/전체 품목 수/공정군별 품목 수를 같은 시간 분할로 비교하고,
# 학습·검증 구간에서 후보를 선정한 뒤 독립 평가 구간에서 재확인한다.
# 확인 전까지 자기소개서·성과 보고서에서 이 가설을 검증된 원인으로 쓰지 않는다.
