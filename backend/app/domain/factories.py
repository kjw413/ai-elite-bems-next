...
FACTORY_DISPLAY_ORDER: tuple[str, ...] = (
    "전사", "남양주", "남양주1", "남양주2", "김해", "광주", "논산", "경산",
)
FACTORY_FILTER_OPTIONS: tuple[str, ...] = (
    "남양주", "남양주1", "남양주2", "김해", "광주", "논산", "경산",
)
...
_FACTORY_DISPLAY_RANK: dict[str, int] = {
    label: rank
    for rank, label in enumerate(
        ("전사", "전체", "ALL", "남양주", "남양주1", "남양주2", "김해", "광주", "논산", "경산")
    )
}


def ordered_factory_labels(factories: object) -> list[str]:
    """Return unique factory labels in the company-wide display order.

    Unknown labels are kept after known factories in their original order so
    newly introduced factories remain visible until the master is updated.
    """
    unique = list(dict.fromkeys(str(factory) for factory in factories))
    input_rank = {factory: rank for rank, factory in enumerate(unique)}
    return sorted(
        unique,
        key=lambda factory: (
            _FACTORY_DISPLAY_RANK.get(factory, len(_FACTORY_DISPLAY_RANK)),
            input_rank[factory],
        ),
    )
