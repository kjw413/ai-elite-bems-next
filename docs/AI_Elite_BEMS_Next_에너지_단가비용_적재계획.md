...
`DB_에너지.xlsx`의 수식 결과를 원본 저장 기준으로 사용합니다. 단, 광주는 판매용
재공품이 생산 실적에 포함되므로 조회 시 운영 생산량 기준으로 다시 계산합니다.

- 일별 냉동·공압·전력·연료·용수 원단위는 엑셀 수식 결과를 그대로 DB에 적재합니다.
- Python은 적재된 원본 값을 덮어쓰지 않습니다.
- 조회·메일·분석 계층은 광주에 한해 운영 생산량(`production_daily` 완제품 + 지정 WIP)을
  분모로 냉동·공압·전력·연료·용수 원단위를 다시 계산합니다. 다른 공장은 저장값을 유지합니다.
- 여러 날짜나 공장을 합칠 때는 `SUM(원단위 × 유효 mix_prod_kg) / SUM(유효 mix_prod_kg)`로
  가중 집계합니다. 광주의 유효 `mix_prod_kg`는 위 운영 생산량입니다.
...
- 시트당 19열. 열 순서는 MIS 화면 순서를 따르고 원단위 수식 열이 뒤에 붙습니다.
...
| O | `냉동원단위[kWh/mix-ton]` | 수식 결과 적재 |
| P | `공압기원단위[kWh/mix-ton]` | 수식 결과 적재 |
| Q | `전력원단위[kWh/mix-ton]` | 수식 결과 적재 |
| R | `연료원단위[N㎥/mix-ton]` | 수식 결과 적재 |
| S | `용수원단위[ton/mix-ton]` | 수식 결과 적재 |
...
`freezing_power_per_ton_kwh` · `air_compressor_per_ton_kwh` · `power_per_ton_kwh` ·
`fuel_per_ton_nm3` · `water_per_ton_ton`은 엑셀 수식 결과의 저장 컬럼입니다.
삭제 대상에 넣지 않습니다. 동기화 후에는 대표 일자의 엑셀 캐시값과 DB 값이 같은지
확인하고, 수식 셀이 비어 있는 신규 행은 원단위 미확정으로 처리합니다.
