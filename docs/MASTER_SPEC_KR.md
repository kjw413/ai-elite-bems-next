...
2. **생산실적 분석** — `production_daily` 기반 계획 vs 실적(품목/보관유형/제품유형별). **구현됨**.
3. **에너지 모니터링** — 전력 사용량 / 연료·용수 사용량 / 원단위 (설비 분해는 전력 화면 내부 섹션).
4. **에너지 절감관리** — 절감 계획 관리 / 절감 실적 현황. **플레이스홀더(미구현)**.
5. **AI 에너지 분석**
...
6. **데이터 업로드** (관리자 전용) — 엑셀 업로드·검증·동기화.
7. **변경 이력** (관리자 전용) — 감사 로그 + 이벤트 메모.
...
구현 완료: 엑셀 업로드·검증, MySQL 저장·초기화, 기동 시 자동 동기화(에너지·생산), 메인 대시보드(이상감지/7일추이/YoY/KPI/CSV), 에너지 모니터링 3화면, **생산실적 분석**, 업로드·변경 이력, AI 실적 보고서, **에너지 사용 예측(v5.3 앙상블, 권한별 탭, 이상감지)**.

미구현(플레이스홀더): 절감 계획 관리, 절감 실적 현황.
