...
> **최종 갱신: 2026-07-18 / 기준 브랜치: `main` / 독립화 Phase 0~4 완료 + legacy 기능 격차 P0·P1·P2 전량 반영(잔여: 실서버 legacy 병행 비교·퇴역)**
>
...
| 이번 세션 완료 (2026-07-18, 페이지·섹션 세부 개선) | 사용자 피드백 기반 UI 개선: **공통** ① 모든 꺾은선 직선(linear)화 ② 에너지·원단위 페이지: 차트는 상시 노출하고 그 아래 데이터 표만 접기 토글(`DataToggle`, 기본 접힘) — 사용자 피드백으로 카드 전체 접기에서 정정. **대시보드** 생산 상징색 연초록(#7bc96f), 월간 이슈+공장별 도넛 한 행(`.dash-pair`). **에너지** 전년대비 꺾은선(금년=상징색), 전년대비(넓게)+공장별(좁게) 나란히, 용수·폐수는 전년대비 크게+비율 작게. **원단위** 에너지원 상징색 차트, 누계 토글을 연간 차트로 이동(월별 usage/ton 원자료 추가), 매트릭스+연간추이 나란히. **생산** '예상 착지'→'월말/연말 예상 실적', 일일 생산량 전체폭, 주요품목+제품믹스 나란히, **신규 '품목 실적 추이·비교' 섹션**(`/production/items`·`/production/item-trend`: 단일 품목 전월비·전년동월비 or 두 품목 비교). **관리자** 메일 송부를 전용 '메일 리포트' 탭으로 분리 + 발송 전 HTML iframe 미리보기(`/mail/preview`). 검증: 테스트 50건·typecheck·빌드·CDP 라이트/다크 렌더링 콘솔 오류 0 |
| 이번 세션 완료 (2026-07-18, 디테일 개선 + 격차 P2 반영) | ① 상징색 사용자 지정 반영: 생산 연노랑 #a4d65e·전력 주황 #f97316·연료 빨강·용수 하늘·폐수 회색(`--chart-production/power/fuel/water/wastewater`) — 7일 비교·전년비 4분면·에너지 차트 일괄 적용 ② 월별 하루 누락 수정: 생산 월별 `[-14:]` 절단 제거(회귀 테스트), 에너지·원단위 기본 창 '당월 1일~기준일' 전환 ③ 메일 발송 카드를 관리자 페이지 데이터·동기화 탭으로 이동(viewer 미노출) ④ **P2 전량**: 대시보드 YTD 구성 도넛 4종(공장 색), 에너지 공장별 비교 라인 토글, 원단위 누계 추이 토글, 생산 연간 월별 전년비·미달/초과 Top 탭·자동 인사이트, 예측 모델 성능 감지(offset) 패널(`/predictions/monitoring`) ⑤ 이식 제외 확정: 임원 요약 인쇄 뷰·예측 입력 편집 그리드(사용자 결정) ⑥ 검증: 테스트 47건·typecheck·빌드·CDP 렌더링 콘솔 오류 0. **격차 반영계획 기준 잔여 항목 없음** ⑦ 백엔드(8000) 점검: FastAPI 정상, 원인은 로컬 MySQL 크래시(anomaly_analysis 손상 — 사내 실DB 가정으로 무시 결정), 구버전 서버 프로세스 정리 후 최신 빌드 재기동 |
| 이번 세션 완료 (2026-07-17, 기능 격차 P1 반영) | GitHub `ai-elite-bems-legacy` public 전환 검증(로컬 스냅샷과 167개 파일 동일) 후 P1 6건 구현: ① 대시보드 월간 전년비 3모드(원단위/사용량/생산량 공장별 4분면 막대, legacy 색 규칙) + 상세 비교 테이블·CSV — `/dashboard` `yoyFactories` + `factory-yoy.tsx` ② 월간 주요 이슈(원단위 악화·생산량 10%↑ 감소, Top 5) ③ 에너지 전력 설비 분해(전체/냉동/공압/기타 라인) ④ 공장별 폐수/용수 비율 ⑤ 원단위 일별·기간별 추이 + 전년대비 **가중 누계**(현재 RawDB 수식 원단위의 생산량 가중평균, `weighted_intensity_yoy`) ⑥ 모델 변수 영향도 API(`/model/feature-importance`)+예측 화면 카드. **임원 요약 인쇄 뷰는 사용자 결정으로 제외.** 잔여 P1: 예측 입력 데이터 편집 그리드. 검증: 테스트 45건·typecheck·빌드·CDP 라이트/다크 렌더링 콘솔 오류 0, `span-all` 레이아웃 정비 |
| 이번 세션 완료 (2026-07-17, 기능 격차 P0 반영) | legacy 스냅샷(`e1fef60~1`) 전수 대조로 격차표 작성 — [기능격차_반영계획](AI_Elite_BEMS_Next_기능격차_반영계획.md) 신설. 사용자 지목 3건 구현: ① **메일 발송** `POST /api/v1/mail/send`(관리자·tools/mail 파이프라인 재사용, 설정누락 503/실패 502) + 대시보드 메일 패널(일간/주간/월간, 발송 확인창) ② **에너지 기간·전년대비**: `/api/v1/energy`에 `date_from/date_to`(최대 731일)·`yoy` 12개월 블록, 화면에 기간 지정 모드 + 전년대비 막대차트·월별 테이블(누계 행은 왜곡 방지 위해 legacy와 달리 **동월 누계**로 개선) ③ **7일 방향 비교**: dashboard trend에 연료·용수·폐수 추가, 4분면 지수 차트(중앙값=100, 1% 임계 방향신호, 신호 칩·테이블·CSV) — `components/seven-day-compare.tsx`. 폐수 신규 색 `--chart-wastewater`(#a21caf/#c026d3) 포함 dataviz 검증기 페어 검증 통과. 검증: 백엔드 테스트 42건(신규 7건)·typecheck·빌드, headless Edge(CDP) 라이트/다크 캡처 — 메일 패널·4분면·누계 테이블 렌더링, 콘솔 오류 0 |
...
| 집 테스트 (2026-07-17) | 사내 fems_db raw `.ibd` 복사본을 로컬 MySQL 9.6에 IMPORT TABLESPACE로 복구 — **9개 중 6개 성공**(production_daily 307,786행 2024-01~2026-07, ai_reports 7건, audit 46,613건 등, CHECK TABLE 전부 OK). `energy_daily`·`prediction_log`·`anomaly_analysis`는 INSTANT ALTER 이력(row versions) 때문에 `.cfg` 없이 IMPORT 불가 → **사내에서 mysqldump로 재반출 필요**(빈 테이블로 재생성해 둠). fems_viewer 계정·backend/.env 구성, 개발 venv에 pandas·openpyxl·mysql-connector-python 보충(서버 PC는 SETUP_LOCAL이 처리). 실DB 연동 검증: Local DB 배지, 생산실적 월별/기간별/연간(**Burn-up 실데이터로 결함 수정 검증**: 실적선 7월 종료), 완전월 계획 규칙, AI 보고서 2026-04 실렌더링, admin 판정, 3100포트 CORS 차단 확인(보호 정상), 콘솔 오류 0 |
| 바로 다음 작업 | ① 사내에서 `mysqldump -u root -p fems_db energy_daily prediction_log anomaly_analysis > fems_rest.sql` 반출 → 집에서 `mysql fems_db < fems_rest.sql`로 나머지 3테이블 복구 ② 실서버에서 RUN_BEMS_NEXT 기동 → legacy 병행(read-only)으로 수치·권한·업로드 동등성 최종 대조 → 메일 dry-run 확인 후 `REGISTER_MAIL_SCHEDULE.bat`로 예약 전환 → legacy 퇴역 |
...
| 일일 메일 리포트 | tools/mail 독립화 + 웹 발송 API·관리자 페이지 발송 카드(2026-07-18 이동) 완료 | 서버 dry-run·실발송 확인 후 스케줄 전환 |
| legacy 기능 격차 | **P0·P1·P2 전량 반영 완료** ([기능격차_반영계획](AI_Elite_BEMS_Next_기능격차_반영계획.md)) | 실서버 수치 대조만 남음 (이식 제외 확정: What-if·임원 요약 인쇄·입력 편집 그리드) |
