...
**무엇을 세는가.** 권한 판정과 같은 클라이언트 IP를 식별자로 씁니다. 사내망에서는
IP가 사실상 PC 1대, 즉 사용자 1명에 대응합니다.

- `access_visit` — (일자, IP) 단위 상세. 같은 IP가 하루에 여러 번 열면 `hit_count`만 증가
- `access_daily` — 일자 단위 롤업. 화면·CSV가 읽는 단일 출처

롤업은 기록할 때마다 상세에서 다시 계산하므로 두 표가 어긋나지 않습니다.

**출처 구분.** `access_daily.source`가 그 날 숫자의 출처를 남깁니다.

| source | 의미 |
| --- | --- |
| `live` | 실제 요청을 받아 적재한 실측 집계 |
| `backfill` | 접속 로깅 도입(2026-09) 이전 구간을 `backend/tools/backfill_access_daily.py`로 채운 값 |

`backfill` 행은 실측이 아닙니다. 두 값이 한 덩어리로 보이면 안 되므로 API 응답
(`GET /api/v1/stats/access`)과 관리자 화면(차트 색·범례·표), CSV의 "출처" 열까지
구분을 그대로 유지합니다. 관리자 화면은 백필 행이 섞인 구간에서 며칠이 실측이고
며칠이 적재분인지 안내 문구로 함께 표시합니다.

**조회.** `GET /api/v1/stats/access?from=&to=`는 관리자 전용이며, 기본 구간은 오늘 포함
최근 90일입니다. 관리자 메뉴의 "접속 통계" 탭이 이 API를 사용합니다.
