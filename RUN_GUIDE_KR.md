...
그 창을 실수로 닫았거나 안 보이는 상태에서 API가 켜져 있는지 확인하려면 `CHECK_API.bat`을 실행한다. 창 없이 안전하게 종료하려면 `STOP_API.bat`을 실행한다 — 8000번 포트의 프로세스가 실제 BEMS API로 응답할 때만 종료하며, 다른 프로그램이면 임의로 끄지 않는다.
...
웹에서도 발송할 수 있다 — 관리자(호스트 PC)로 접속해 **관리자·현장 메모 >
데이터·동기화** 탭의 "에너지 리포트 메일 발송" 카드에서 일간/주간/월간을 선택해
즉시 발송한다(`POST /api/v1/mail/send`, 조회 사용자에게는 노출·허용되지 않음).
CLI와 같은 빌더·발송 파이프라인을 사용하므로 산출물은 동일하다.
...
`BEMS API :8000` 창을 닫았는지 기억이 안 나거나 다른 사람이 실행해 창이 안 보이는 경우, 매번 작업 관리자에서 뒤질 필요 없이 다음 두 스크립트로 확인·종료한다.

| 스크립트 | 동작 |
|---|---|
| `CHECK_API.bat` | 8000·3000 포트 사용 여부와 PID를 확인하고, `/api/v1/session` 응답으로 실제 BEMS API인지 검증한다. |
| `STOP_API.bat` | 8000번 포트 프로세스가 BEMS API로 응답할 때만 안전하게 종료한다. 다른 프로그램이면 종료하지 않고 경고만 표시한다. |

종료 후 다시 켜려면 `RUN_BEMS_NEXT.bat`를 실행한다.
