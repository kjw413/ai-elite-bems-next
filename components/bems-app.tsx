  ...
  return <>
    ...
    <section className="content-grid">
      ...
      <article className="card chart-card span-all"><CardTitle title={trendTitle} meta="ton"><CsvButton filename={`production_${mode}_${(data.dateFrom ?? "").replaceAll("-", "")}`} rows={data.daily} columns={csvColumns} labels={csvLabels}/></CardTitle>
        <Chart><ComposedChart data={data.daily}><CartesianGrid vertical={false}/><XAxis dataKey="date" interval="preserveStartEnd" minTickGap={18}/><YAxis/><Tooltip {...tooltipStyle} formatter={numberFormatter}/>{cat2ActiveKeys.filter(key => !productionLegend.isHidden(key)).map((key, index, visible) => <Bar key={key} dataKey={key} stackId="a" fill={palette.cat2[key]} stroke="var(--card)" strokeWidth={1} maxBarSize={22} radius={index === visible.length - 1 ? [4,4,0,0] : undefined}/>)}{showUtilityProd && !productionLegend.isHidden("utilityProd") && <Line type="linear" dataKey="utilityProd" name="유틸리티 사용 총 생산량" stroke="var(--chart-production)" strokeWidth={2} dot={seriesDot("var(--chart-production)")} activeDot={{ r: 5 }} connectNulls/>}</ComposedChart></Chart>
