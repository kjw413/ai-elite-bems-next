...
// API가 200을 돌려줘도 실데이터가 비어 있는 상태(예: DB 동기화 전, 빈 테이블)를 감지한다.
// 이때는 화면을 비워 두는 대신 예시 데이터로 폴백해 UI를 확인할 수 있게 한다.
// 생산 실적처럼 일부만 채워진 부분 적재 상태에서도, 각 화면의 '주 콘텐츠'가 비면 폴백한다.
function isEmptyPayload(screen: Screen, data: AnyData): boolean {
  switch (screen) {
    case "dashboard":
      // 생산량 KPI는 production_daily에서 오므로 제외하고, 에너지 원단위 지표로 판정.
      return !(data?.metrics ?? []).some((m: AnyData) => m?.id !== "production" && m?.value != null);
    case "energy":
      return !(data?.daily?.length);
    case "intensity":
      return !(data?.monthly ?? []).some((m: AnyData) => m?.current != null || m?.previous != null);
    case "production":
      return !((data?.summary?.actual ?? 0) > 0);
    case "prediction":
      return !(data?.latest?.length);
    default:
      return false;
  }
}
...
function Energy({ data, mode, onModeChange, rangeFrom, rangeTo, onRangeChange }: {
  ...
  return <><div className="segmented" role="group" aria-label="에너지 지표 선택">{Object.entries(energyMetricLabels).map(([id,label])=><button type="button" className={metric===id?"active":""} aria-pressed={metric===id} onClick={()=>setMetric(id)} key={id}>{label}</button>)}</div>
    ...
    <section className="content-grid">
      <CollapsibleCard className="wide" title={comparing ? "일별 사용 추이 · 공장별 비교" : metric === "power" ? "일별 사용 추이 · 설비 분해" : "일별 사용 추이"} meta={units[metric]} side={comparing
        ...
        : <CsvButton filename={`energy_daily_${metric}`} rows={data.daily} columns={["date","power","freezing","compressor","other","fuel","water","wastewater"]} labels={{date:"일자",power:"전력(MWh)",freezing:"냉동(MWh)",compressor:"공압(MWh)",other:"기타(MWh)",fuel:"연료(천 Nm³)",water:"용수(천 ton)",wastewater:"폐수(천 ton)"}}/>}><Chart>{comparing
        ...
        : <AreaChart data={data.daily}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date" interval="preserveStartEnd" minTickGap={18}/><YAxis/><Tooltip {...tooltipStyle} formatter={numberFormatter}/><Area type="linear" dataKey={metric} stroke={energyMetricColors[metric]} strokeWidth={3} fill={energyMetricColors[metric]} fillOpacity={0.1}/></AreaChart>}</Chart></CollapsibleCard>
      <CollapsibleCard title="설비 구성" meta={summaryMeta}>{data.equipment?.map((r:AnyData)=><div className="progress" key={r.name}><div><span>{r.name}</span><b>{fmt(r.value)}%</b></div><i><em style={{width:`${r.value}%`}}/></i></div>)}</CollapsibleCard>
      <CollapsibleCard className="wide" title={`전년대비 ${energyMetricLabels[metric]} 사용량`} meta={`${data.yoyYear ?? ""}년 vs 전년 · ${yoyUnit}`} side={<CsvButton filename={`energy_yoy_${metric}_${data.yoyYear ?? ""}`} rows={yoyCsvRows} columns={["month","previous","current","diff","diffPct"]} labels={{month:"월",previous:`전년(${yoyUnit})`,current:`금년(${yoyUnit})`,diff:`증감량(${yoyUnit})`,diffPct:"증감률(%)"}}/>}><Chart><LineChart data={yoyTable.rows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month"/><YAxis/><Tooltip {...tooltipStyle} formatter={numberFormatter}/><Legend/><Line type="linear" dataKey="previous" name="전년" stroke={palette.previous} strokeWidth={2} dot={{ r: 2.5 }} connectNulls/><Line type="linear" dataKey="current" name="금년" stroke={energyMetricColors[metric]} strokeWidth={3} dot={{ r: 3 }} connectNulls/></LineChart></Chart>
        ...
        </tbody></table></div></CollapsibleCard>
      {isWater
        ? <CollapsibleCard title="공장별 폐수/용수 비율" meta={`${summaryMeta} · 낮을수록 양호`} side={<CsvButton filename={`wastewater_ratio_${data.dateFrom ?? ""}`} rows={wasteRatioRows} columns={["factory","water","wastewater","ratio"]} labels={{factory:"공장",water:"용수(천 ton)",wastewater:"폐수(천 ton)",ratio:"폐수/용수"}}/>}><Chart><BarChart data={wasteRatioRows}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="factory"/><YAxis/><Tooltip {...tooltipStyle} formatter={numberFormatter}/><Bar dataKey="ratio" name="폐수/용수 비율" fill={energyMetricColors.wastewater} radius={[4,4,0,0]}/></BarChart></Chart></CollapsibleCard>
        : <CollapsibleCard title="공장별 사용량" meta={`${summaryMeta} 누계`} side={<CsvButton filename="energy_factories" rows={data.factories} columns={["factory","power","fuel","water","wastewater"]} labels={{factory:"공장",power:"전력(MWh)",fuel:"연료(천 Nm³)",water:"용수(천 ton)",wastewater:"폐수(천 ton)"}}/>}><DataTable rows={data.factories} columns={["factory",metric]} labels={{factory:"공장",[metric]:units[metric]}}/></CollapsibleCard>}
      {isWater && <CollapsibleCard className="span-all" title="공장별 사용량" meta={`${summaryMeta} 누계`} side={<CsvButton filename="energy_factories" rows={data.factories} columns={["factory","power","fuel","water","wastewater"]} labels={{factory:"공장",power:"전력(MWh)",fuel:"연료(천 Nm³)",water:"용수(천 ton)",wastewater:"폐수(천 ton)"}}/>}><DataTable rows={data.factories} columns={["factory",metric]} labels={{factory:"공장",[metric]:units[metric]}}/></CollapsibleCard>}
    </section></>;
}
...
function Intensity({ data, metric, onMetricChange, mode, onModeChange, rangeFrom, rangeTo, onRangeChange }: {
  ...
  return <><div className="segmented" role="group" aria-label="원단위 지표 선택">{intensityMetrics.map(item=><button type="button" key={item.id} className={metric===item.id?"active":""} aria-pressed={metric===item.id} onClick={()=>onMetricChange(item.id)}>{item.label}</button>)}</div>
    ...
    <section className="content-grid">
      <CollapsibleCard className="span-all" title="일별 원단위 추이" meta={`${data.unit} · 생산 실적 있는 날만 표시`} side={<CsvButton filename={`intensity_daily_${metric}_${(data.dateFrom ?? "").replaceAll("-","")}`} rows={dailySeries} columns={["date","value"]} labels={{date:"일자",value:`원단위(${data.unit})`}}/>}><Chart><LineChart data={dailySeries}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="date" interval="preserveStartEnd" minTickGap={18}/><YAxis domain={["auto","auto"]}/><Tooltip {...tooltipStyle} formatter={numberFormatter}/><Line type="linear" dataKey="value" name={`원단위(${data.unit})`} stroke={metricColor} strokeWidth={2.5} connectNulls={false} dot={{ r: 2.5 }}/></LineChart></Chart></CollapsibleCard>
      <CollapsibleCard className="wide" title={`${data.year}년 원단위 추이${showCumulative ? " · 누계" : ""}`} meta={data.unit} side={<><label className="check-toggle"><input type="checkbox" checked={showCumulative} onChange={event => setShowCumulative(event.target.checked)}/>누계 추이 보기</label><CsvButton filename={`intensity_monthly_${metric}_${data.year}`} rows={monthlySeries} columns={["month","previous","target","current"]} labels={{month:"월",previous:`전년(${data.unit})`,target:`목표(${data.unit})`,current:`금년(${data.unit})`}}/></>}><Chart><LineChart data={monthlySeries}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="month"/><YAxis/><Tooltip {...tooltipStyle} formatter={numberFormatter}/><Legend/><Line type="linear" dataKey="previous" name="전년" stroke={palette.previous} connectNulls/><Line type="linear" dataKey="target" name="목표" stroke={palette.target} strokeDasharray="5 4" connectNulls/><Line type="linear" dataKey="current" name="금년" stroke={metricColor} strokeWidth={3} connectNulls/></LineChart></Chart>
        ...
        <p className="quad-caption">누계 추이 보기는 각 월을 1월부터의 가중 평균(Σ사용량 ÷ Σ생산톤)으로 다시 그립니다.</p></CollapsibleCard>
      <CollapsibleCard title="공장 효율 매트릭스" meta="MTD 기준" side={<CsvButton filename={`intensity_matrix_${metric}`} rows={data.matrix} columns={["factory","current","previous","change"]} labels={{factory:"공장",current:`금년(${data.unit})`,previous:`전년(${data.unit})`,change:"증감률(%)"}}/>}><DataTable rows={data.matrix} columns={["factory","current","previous","change"]} labels={{factory:"공장",current:"금년",previous:"전년",change:"증감률(%)"}}/></CollapsibleCard>
    </section></>;
}
...
// 접이식 카드 (에너지 사용량·원단위 페이지 공통, 기본 접힘) — summary 우측 컨트롤
// 클릭 시 preventDefault로 접힘 토글을 막아 CSV 버튼 등이 독립 동작한다.
function CollapsibleCard({ title, meta, className = "", side, defaultOpen = false, children }: {
  title: string; meta?: string; className?: string; side?: React.ReactNode; defaultOpen?: boolean; children: React.ReactNode;
}) {
  return <details className={`card chart-card collapsible ${className}`} open={defaultOpen}>
    <summary className="card-title"><h3>{title}</h3><div className="card-title-side" onClick={event => event.preventDefault()}>{side}{meta && <span>{meta}</span>}</div></summary>
...
export function BemsApp() {
  ...
  // 예시 데이터를 쓰게 된 사유 — "offline"(API 연결 실패) / "empty"(실데이터 없음) / ""(실데이터).
  const [demoReason,setDemoReason]=useState<"offline"|"empty"|"">("offline");
  ...
  useEffect(()=>{
    ...
    apiGet(`${endpoint[screen]}?${suffix}`,fallback,controller.signal).then(r=>{
      if (r.live && isEmptyPayload(screen, r.data)) {
        // 200이지만 실데이터가 비어 있으면 예시 데이터로 폴백한다.
        setData(fallback); setLive(false); setDemoReason("empty");
      } else {
        setData(r.data); setLive(r.live); setDemoReason(r.live ? "" : "offline");
      }
      setLoading(false);
    }).catch(()=>{});
