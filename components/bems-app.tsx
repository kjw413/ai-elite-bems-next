...
const titles: Record<Screen, [string, string]> = {
  ...
  savings: ["에너지 절감", "연초 계획과 월 실적(MTD)·누계(YTD), 실제 사용량 감소를 확인합니다."],
...
function Energy({ data, factory, requestedDate, view, onViewChange, mode, onModeChange, rangeFrom, rangeTo, onRangeChange }: {
  data: AnyData; factory: string; requestedDate: string; view: EnergyView; onViewChange: (view: EnergyView) => void; mode: EnergyMode; onModeChange: (mode: EnergyMode) => void;
...
// "전사(경산 제외)" — 경산은 실적 시작일이 2026-04라 5개 공장과 전년비가 동일 기준
// 비교되지 않는다(경산만 전년 실적이 없어 증가분이 통째로 얹힘). 기본 "전사"는
// 그대로 두고, 이 옵션을 고르면 백엔드가 경산을 뺀 5개 공장만 집계해 돌려준다.
const FACTORY_GROUPS: { label: string; options: string[] }[] = [
  { label: "전사", options: ["전사", "전사(경산 제외)"] },
